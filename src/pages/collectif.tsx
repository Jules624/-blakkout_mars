            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
              {members.map((member, index) => (
                <motion.div
                  ...
                  className="group overflow-hidden rounded-md border border-blakkout-primary bg-blakkout-background/50 backdrop-blur-sm"
                >
                  <div className="relative aspect-square overflow-hidden bg-blakkout-muted">
                    {/* Placeholder pour image */}
                    <div className="flex h-full items-center justify-center bg-blakkout-muted">
                      <span className="font-mono text-xs text-blakkout-primary">{member.alias || member.name}</span>
                    </div>
                    
                    {/* Overlay avec effet glitch */}
                    <div className="absolute inset-0 bg-gradient-to-b from-transparent to-blakkout-background/80 opacity-70"></div>
                    
                    {/* Badge rôle */}
                    <div className="absolute left-4 top-4 rounded-full bg-blakkout-primary px-2 py-1 font-mono text-xs text-blakkout-background">
                      {member.role === 'founder' ? 'FONDATEUR' :
                       member.role === 'dj' ? 'DJ / PRODUCTEUR' :
                       member.role === 'vj' ? 'VJ / ARTISTE VISUEL' :
                       member.role === 'dev' ? 'DÉVELOPPEUR' :
                       'COMMUNICATION'}
                    </div>
                  </div>
                  
                  <div className="p-6">
                    <h3 className="mb-1 font-display text-xl text-blakkout-primary">{member.name}</h3>
                    {member.alias && (
                      <div className="mb-2 font-mono text-sm text-blakkout-accent">{member.alias}</div>
                    )}
                    <p className="mb-4 font-mono text-sm text-blakkout-foreground">{member.bio}</p>
                    
                    {/* Liens sociaux */}
                    {member.socialLinks && (
                      <div className="mt-4 flex space-x-3">
                        {member.socialLinks.instagram && (
                          <a href={member.socialLinks.instagram} target="_blank" rel="noopener noreferrer" className="text-blakkout-foreground hover:text-blakkout-primary">
                        ...
                        {member.socialLinks.soundcloud && (
                          <a href={member.socialLinks.soundcloud} target="_blank" rel="noopener noreferrer" className="text-blakkout-foreground hover:text-blakkout-primary">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                              <path d="M2 12h2l1-3 1 3h2l1-3 1 3h2l1-3 1 3h2l1-3 1 3h2l1-3 1 3h2"></path>
                            </svg>
                          </a>
                        )}
                        {member.socialLinks.github && (
                          <a href={member.socialLinks.github} target="_blank" rel="noopener noreferrer" className="text-blakkout-foreground hover:text-blakkout-primary">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        {member.socialLinks.website && (
                          <a href={member.socialLinks.website} target="_blank" rel="noopener noreferrer" className="text-blakkout-foreground hover:text-blakkout-primary">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
